...
# === Ensure .env exists ===
if [ ! -f ".env" ]; then
  echo "❌ Missing .env (must contain AOC_ORG_ID, AOC_MODULATION_TOKEN, MODEL_TEMPLATE_ID, BASE_URL)"; exit 1
fi
...
if [ -z "${AOC_ORG_ID:-}" ]; then
  echo "❌ AOC_ORG_ID missing in .env"; exit 1
fi
if [ -z "${AOC_MODULATION_TOKEN:-}" ]; then
  echo "❌ AOC_MODULATION_TOKEN missing in .env"; exit 1
fi
